...
function AddUnitComponent({ setAddUnit, setLoading, setSnackBar }) {
  ...
  const { updateUnits, unit, setUnit, siteListOptions } = useDashboardStore();

  const [name, setName] = useState(unit?.name);
  const [site, setSite] = useState(unit?.site?.id || null);
  const [bedroom, setBedroom] = useState(unit?.bedroom);
  const [bathroom, setBathroom] = useState(unit?.bathroom);
  const [balcony, setBalcony] = useState(unit?.balcony || null);
  ...
  if (siteListOptions.length == 0) return <Loading />;
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={8}>
                  <FormControl size="small" fullWidth>
                    ...
                    >
                      <MenuItem value={null}>Select Site</MenuItem>
                ...
                <Grid item xs={2} mr={-2}>
                  ...
                  >
                    <Typography>
                      {site &&
                        getCurrency(
                          siteListOptions.find(
                            (siteOption) => siteOption.id == site
                          )?.realEstate.currency
                        )?.abbr}
                    </Typography>
                  </Stack>
